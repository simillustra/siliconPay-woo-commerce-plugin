...
`;
    ...
    function wcSiliconPayFormHandler() {
        ...
        var payment = wc_siliconpay_app_params.payment_channel;
        var method;
        ...
        if (currency === "USD" || payment === "Credit_Debit_Card") {
            method = "card_payment";
            currency = "USD"
        } else {
            method = "mobile_money";
        ...
        $.ajax(woo_sp_settings)
            .done(function (response) {
                ...
                if (response_link !== null) {
                    ...
                    var strWindowFeatures = "location=on,height=570,width=520,scrollbars=on,status=on,resizable=on";
                    var windowObjectReference = window.open(response_link, "_blank", strWindowFeatures);
                    if (windowObjectReference === null) {
                        // setTimeout(() => {
                        window.location.href = response_link;
                        // }, 10000);
                    }
                }
            })
